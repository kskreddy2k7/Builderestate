...
let AiService = AiService_1 = class AiService {
    ...
    async estimateCost(dto, userId) {
        ...
        let tier = 'TIER3';
        if (TIER1_CITIES.some((c) => cityNorm.toLowerCase().includes(c.toLowerCase())))
            tier = 'TIER1';
        else if (TIER2_CITIES.some((c) => cityNorm.toLowerCase().includes(c.toLowerCase())))
            tier = 'TIER2';
        const rates = COST_RATES[tier][dto.specifications];
        let multiplier = 1;
        if (dto.floors > 10)
            multiplier += 0.05 * Math.floor((dto.floors - 10) / 5);
        if (dto.basementRequired)
            multiplier += 0.08;
        if (dto.parkingRequired)
            multiplier += 0.05;
        if (dto.buildingType === 'COMMERCIAL')
            multiplier += 0.10;
        ...
        await this.prisma.aIRequest.create({
            data: {
                userId, type: 'COST_ESTIMATE', input: dto,
                output: result, model: 'buildestate-cost-model-v1',
    ...
    // ─── Property Valuation ───────────────────────────────────────────────────
    async valuateProperty(dto, userId) {
        ...
        await this.prisma.aIRequest.create({
            data: {
                userId, type: 'VALUATION', input: dto,
                output: result, model: 'buildestate-valuation-v1',
                status: 'COMPLETED', completedAt: new Date(),
            },
        });
        return result;
    ...
    // ─── Risk Detection ───────────────────────────────────────────────────────
    async detectProjectRisks(dto, userId) {
        ...
        if (project.budget) {
            const overrunHeads = project.budget.heads.filter((h) => Number(h.spentAmount) > Number(h.allocatedAmount) * 0.9);
    ...
    async scoreLead(leadId, userId) {
        ...
        if (hasCallActivity) {
            factors.push({ factor: 'Call made', impact: 10, description: 'Lead has been contacted by phone' });
            score += 10;
        }
        if (hasSiteVisit) {
            factors.push({ factor: 'Site visited', impact: 20, description: 'Completed site visit indicates strong intent' });
            score += 20;
        }
        if (lead.budgetMin && lead.budgetMax) {
            factors.push({ factor: 'Budget defined', impact: 10, description: 'Has clear budget range' });
            score += 10;
        }
        if (lead.email) {
            factors.push({ factor: 'Email provided', impact: 5, description: 'Contact completeness' });
            score += 5;
        }
        if (lead.preferredLocations?.length) {
            factors.push({ factor: 'Location preference', impact: 5, description: 'Has specific location preferences' });
            score += 5;
        }
        if (lead.source === 'REFERRAL') {
            factors.push({ factor: 'Referral source', impact: 15, description: 'Referral leads close 3x more often' });
            score += 15;
        }
        if (daysSinceCreation > 30 && lead.stage === 'NEW') {
            factors.push({ factor: 'Stale lead', impact: -10, description: 'No activity in 30+ days' });
            score -= 10;
        }
    ...
    async chat(dto, userId) {
        const requestId = await this.prisma.aIRequest.create({
            data: {
                userId, type: 'DOC_ANALYSIS', input: { message: dto.message },
...
Always respond in the context of Indian real estate. Mention INR for currencies.`;
            ...
            throw new common_1.ServiceUnavailableException('AI service temporarily unavailable');
        }
    }
    // ─── History ──────────────────────────────────────────────────────────────
    ...
    async getRiskFlags(projectId) {
        return this.prisma.riskFlag.findMany({
            where: { projectId, status: { in: ['ACTIVE', 'ACKNOWLEDGED'] } },
            orderBy: [{ severity: 'desc' }, { detectedAt: 'desc' }],
        });
    }
    getDefaultRateForCity(city) {
        const cityNorm = city.toLowerCase();
        if (TIER1_CITIES.some((c) => cityNorm.includes(c.toLowerCase())))
            return 7500;
        if (TIER2_CITIES.some((c) => cityNorm.includes(c.toLowerCase())))
            return 5500;
        return 4000;
    }
};
