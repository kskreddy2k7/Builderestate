...
let CrmService = CrmService_1 = class CrmService {
    ...
    async createLead(dto, user) {
        if (!user.orgId)
            throw new common_1.ForbiddenException('Must belong to an organisation');
        // Duplicate check within org
        const duplicate = await this.prisma.lead.findFirst({
            where: { phone: dto.phone, orgId: user.orgId, stage: { not: client_1.LeadStage.JUNK } },
        });
        if (duplicate)
            throw new common_1.ConflictException(`Lead with phone ${dto.phone} already exists in your CRM`);
        const lead = await this.prisma.lead.create({
            data: {
                name: dto.name, email: dto.email, phone: dto.phone,
                alternatePhone: dto.alternatePhone,
                source: dto.source, stage: dto.stage ?? client_1.LeadStage.NEW,
    ...
    async getLeads(user, dto) {
        const { page = 1, limit = 20, sortBy = 'createdAt', sortOrder = 'desc', search, stage, source, assignedToId, projectId, overdueFollowUp } = dto;
        const where = { orgId: user.orgId };
        // Non-admins see only their own leads
        if (!user.roles.includes(client_1.UserRole.ADMIN) && !user.roles.includes(client_1.UserRole.SUPER_ADMIN)) {
        ...
        if (overdueFollowUp) {
            ...
            where.stage = { notIn: [client_1.LeadStage.BOOKED, client_1.LeadStage.LOST, client_1.LeadStage.JUNK] };
        }
        const orderBy = sortBy === 'score' ? { score: sortOrder }
    ...
    async updateLead(id, dto, user) {
        ...
        const updated = await this.prisma.lead.update({
            ...
            data: {
                ...
                ...(dto.stage === client_1.LeadStage.BOOKED || dto.stage === client_1.LeadStage.LOST ? { closedAt: new Date() } : {}),
            },
        });
    ...
    async transferLead(id, dto, user) {
        const lead = await this.prisma.lead.findUnique({ where: { id } });
        if (!lead)
            throw new common_1.NotFoundException('Lead not found');
        if (lead.orgId !== user.orgId)
        ...
        return this.redis.cached(cacheKey, async () => {
            ...
            const [stageCounts, sourceCounts, overdue, recentLeads] = await Promise.all([
                ...
                this.prisma.lead.count({
                    where: { ...where, nextFollowUpDate: { lt: new Date() }, stage: { notIn: [client_1.LeadStage.BOOKED, client_1.LeadStage.LOST, client_1.LeadStage.JUNK] } },
                }),
                this.prisma.lead.findMany({
            ...
            const stageOrder = [client_1.LeadStage.NEW, client_1.LeadStage.CONTACTED, client_1.LeadStage.INTERESTED, client_1.LeadStage.SITE_VISIT_SCHEDULED, client_1.LeadStage.SITE_VISIT_DONE, client_1.LeadStage.NEGOTIATION, client_1.LeadStage.BOOKED, client_1.LeadStage.LOST];
    ...
    async getActivities(leadId, page = 1, limit = 30) {
        const [items, total] = await Promise.all([
    ...
    // ─── Site Visits ──────────────────────────────────────────────────────────
    async scheduleSiteVisit(leadId, dto, user) {
        const lead = await this.prisma.lead.findUnique({ where: { id: leadId } });
        if (!lead)
            throw new common_1.NotFoundException('Lead not found');
        ...
        if (lead.stage === client_1.LeadStage.NEW || lead.stage === client_1.LeadStage.CONTACTED || lead.stage === client_1.LeadStage.INTERESTED) {
            await this.prisma.lead.update({ where: { id: leadId }, data: { stage: client_1.LeadStage.SITE_VISIT_SCHEDULED } });
        }
        await this.prisma.leadActivity.create({
            data: {
    ...
    async completeSiteVisit(visitId, dto, user) {
        ...
        if (dto.status === 'COMPLETED' && visit.lead.stage === client_1.LeadStage.SITE_VISIT_SCHEDULED) {
            await this.prisma.lead.update({ where: { id: visit.leadId }, data: { stage: client_1.LeadStage.SITE_VISIT_DONE } });
    ...
    // ─── Commissions ──────────────────────────────────────────────────────────
    async createCommission(dto, user) {
        ...
        if (existing)
            throw new common_1.ConflictException('Commission already exists for this booking');
    ...
    async getCommissions(user, page = 1, limit = 20, status) {
    ...
    async updateCommissionStatus(id, dto, user) {
        ...
        const isAdmin = user.roles.includes(client_1.UserRole.ADMIN) || user.roles.includes(client_1.UserRole.SUPER_ADMIN);
        if (!isAdmin && dto.status === 'APPROVED')
            throw new common_1.ForbiddenException('Only admins can approve commissions');
        ...
        if (dto.status === 'APPROVED') {
            await this.notifications.create({
                ...
                type: client_1.NotificationType.COMMISSION_APPROVED,
    ...
    async recordCommissionPayment(dto, user) {
        const commission = await this.prisma.commission.findUnique({ where: { id: dto.commissionId } });
        if (!commission)
            throw new common_1.NotFoundException('Commission not found');
        if (commission.status !== client_1.CommissionStatus.APPROVED)
            throw new common_1.BadRequestException('Commission must be approved before payment');
        return this.prisma.commission.update({
            where: { id: dto.commissionId },
            data: {
                status: client_1.CommissionStatus.PAID, paidAt: new Date(),
    ...
    async getCustomers(orgId, page = 1, limit = 20, search) {
        const where = {
            orgId,
            ...(search && {
                OR: [
    ...
    // ─── Private helpers ──────────────────────────────────────────────────────
    calculateInitialScore(dto) {
        let score = 20;
        if (dto.email)
            score += 10;
        if (dto.budgetMin && dto.budgetMax)
            score += 15;
        if (dto.preferredLocations?.length)
            score += 10;
        if (dto.preferredBhk?.length)
            score += 10;
        if (dto.source === client_1.LeadSource.REFERRAL)
            score += 15;
        if (dto.source === client_1.LeadSource.WALK_IN)
            score += 10;
        if (dto.propertyId || dto.projectId)
            score += 10;
        return Math.min(score, 100);
    }
    async updateLeadScore(leadId, activityType) {
        const increments = {
